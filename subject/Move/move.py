...
plt.bar(x, summary['遷入'], width=0.4, label='遷入', align='center')
plt.bar([i + 0.4 for i in x], summary['遷出'], width=0.4, label='遷出', align='center')
plt.xticks([i + 0.2 for i in x], custom_order, rotation=45)
plt.ylabel('人口數')
plt.title('2024年各縣市遷入遷出人口統計')
plt.legend()
plt.tight_layout()
plt.savefig('2024縣市人口遷移總人數.png')
plt.show()
